...
//Hui
static bool featureOrNOT(TriMesh *normal, int f, int e)
{
	int v1 = normal->faces[f][NEXT(e)], v2 = normal->faces[f][PREV(e)];

	point p1,p2;
	p1 = normal->vertices[v1];
	p2 = normal->vertices[v2];
...
// Insert a new vertex
//Hui Wang, Sep. 2, 2012
static point insert_vertmy(TriMesh *mesh, TriMesh *normal, int f, int e)
{
	int v1 = mesh->faces[f][NEXT(e)], v2 = mesh->faces[f][PREV(e)];
	...
	point M,N;
	M = normal->vertices[v1];
	N = normal->vertices[v2];
   
	point direction;
    direction = M % N;
	float length = 1.0 / sqrt(direction ^ direction);
	...
	if(abs((M[1] * N[2] - M[2] * N[1])) > 1e-16)
	{
	  p[0] = 0;
	  p[1] = (a * N[2] - b * M[2]) / (M[1] * N[2] - M[2] * N[1] + 1e-16);
	  p[2] = (M[1] * b - N[1] * a) / (M[1] * N[2] - M[2] * N[1] + 1e-16);
	}
	else if(abs(M[0] * N[2] - M[2] * N[0]) > 1e-16)
	{
		p[1] = 0;
		p[0] = (a * N[2] - b * M[2]) / (M[0] * N[2] - M[2] * N[0] + 1e-16);
		p[2] = (b * M[0] - a * N[0]) / (M[0] * N[2] - M[2] * N[0] + 1e-16);
	}
    else if(abs((M[0] * N[1] - M[1] * N[0])) > 1e-16)
	{
		p[2] = 0;
		p[0] = (a * N[1] - b * M[1]) / (M[0] * N[1] - M[1] * N[0] + 1e-16);
		p[1] = (M[0] * b - N[0] * a) / (M[0] * N[1] - M[1] * N[0] + 1e-16);
	}
	else
	{
       return p1;
    ...
    //Point O (p[0], p[1], p[2]);
	//Vector V(direction[0], direction[1], direction[2]);
    //Line line(O - 10 * V, O + 10 * V);

    
	//Point P;
	//P = line.projection(Point(p1[0], p1[1], p1[2]));
	
    point result = p + ((p1 - p) ^ direction) * direction;
    //result[0] = P.x();
	//result[1] = P.y();
	//result[2] = P.z();
	if(((result - p1) ^ (result - p1)) > 5 * ((A - B) ^ (A - B)))
       result = p1;
	return result;
}

//Hui Wang, Sep. 2, 2012
void subdivMy(TriMesh *mesh, TriMesh *normal, int scheme, std::vector<int> &featureIndex, std::vector<point> &featurePoint)
{
	bool have_col = !mesh->colors.empty();
	bool have_conf = !mesh->confidences.empty();
	mesh->flags.clear();
	...
	for (int i = 0; i < nf; i++) {
		for (int j = 0; j < 3; j++) {
			...
			//Hui
			if(featureOrNOT(normal,i,j))
			{
				featureIndex.push_back(newverts[i][j] + 1);
				featurePoint.push_back(insert_vertmy(mesh, normal,i,j));
			}
			if (ae != -1) {
				if (mesh->across_edge[ae][0] == i)
					newverts[ae][0] = newverts[i][j];
